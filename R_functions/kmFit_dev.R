...
#' #' @param contrast_var 2 column dataframe specifying contrasts in "contrast_ref" and "contrast_lvl".
...
kmFit_dev <- function(dat=NULL, kin=NULL, patientID="ptID", libraryID="libID",
                  ...
                  metrics = FALSE,
                  run_contrast = FALSE, contrast_var = NULL, contrast_spec=NULL,
  ...
  fit.results <- data.table::rbindlist(fill=TRUE,
                    foreach::foreach(gene=unique(to.model.ls[["to.model"]]$rowname),
                                     .packages = c("dplyr","magrittr","stats","broom","lme4",
                                                   ...
                                                   "data.table","foreach","doParallel", "kimma"),
                                     .export = c("kmFit_contrast_dev", "contrast_spec", "contrast_var",
                                                 "genotype_name", "use_weights", "metrics",
                                                 "model_lm", "model_lme", "patientID")) %dopar% {
    ...
    #Wrap model run in error catch to allow loop to continue even if a single model fails
     results.lm.ls <- tryCatch({
       kimma:::kimma_lm(model_lm, to.model.gene, gene, use_weights, metrics)
     }, error=function(e){
    ...
    if(run_lme){
      ...
      results.lme.ls <- tryCatch({
        kimma:::kimma_lme(model_lme, to.model.gene, gene, use_weights, metrics)
        }, error=function(e){
    ...
    if(run_lmerel){
      ...
      results.kin.ls <- tryCatch({
        kimma:::kimma_lmerel(model_lme, to.model.gene, gene, to.model.ls[["kin.subset"]],
    ...
    if(run_contrast){
      if(!is.null(results.lm.ls[["results"]])){
        contrast.lm <- tryCatch({
          kmFit_contrast_dev(results.lm.ls[["fit"]],
                             contrast_var, contrast_spec, to.model.gene,
                             genotype_name) %>%
      ...
      if(!is.null(results.lme.ls[["results"]])){
        contrast.lme <- tryCatch({
          kmFit_contrast_dev(results.lme.ls[["fit"]], contrast_var, contrast_spec, to.model.gene,
                         genotype_name) %>%
      ...
      if(!is.null(results.kin.ls[["results"]])){
        contrast.kin <- tryCatch({
          kmFit_contrast_dev(results.kin.ls[["fit"]], contrast_var, contrast_spec, to.model.gene,
                         genotype_name) %>%
